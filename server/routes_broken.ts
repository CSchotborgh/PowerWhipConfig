  ...
  app.post('/api/excel/process-configurator', async (req, res) => {
    try {
      ...
      const results = inputPatterns.map((pattern: string) => {
        ...
        // Optimized search: limit to first sheet and first 50 rows for speed
        const sheetName = workbook.SheetNames[0];
        const worksheet = workbook.Sheets[sheetName];
        const jsonData = XLSX.utils.sheet_to_json(worksheet, { header: 1 });
        
        (jsonData as any[][]).slice(0, 50).forEach((row: any[], rowIdx) => {
        ...
        return {
          ...
          foundInSheets: [...new Set(matchedRows.map((r: any) => r.sheetName))],
