...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Get sheet data for Excel File Viewer
  app.get('/api/excel/sheet-data/:fileId/:sheetName', async (req, res) => {
    try {
      ...
      const worksheet = fileData.workbook.Sheets[decodeURIComponent(sheetName)];
      if (!worksheet) {
        return res.status(404).json({ error: 'Sheet not found' });
      }
      ...
      // Create AG-Grid compatible column definitions
      const headers = jsonData[0] || [];
      const columnDefs = headers.map((header: any, index: number) => ({
        field: `col_${index}`,
      ...
      const rowData = jsonData.slice(1).map((row: any[]) => {
        ...
        headers.forEach((_, index) => {
          rowObject[`col_${index}`] = row[index] || '';
        });
        return rowObject;
      });
  ...
  app.delete('/api/excel/delete/:fileId', async (req, res) => {
    try {
      const { fileId } = req.params;
  ...
  // Export Excel file with modifications
  app.post('/api/excel/export', async (req, res) => {
    try {
      const { fileName, sheetName, data, columns } = req.body;
      ...
      const worksheet = XLSX.utils.aoa_to_sheet([
        columns, // headers
        ...data.map((row: any) => columns.map((col: string) => row[col] || ''))
      ]);
